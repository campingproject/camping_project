import { CALENDAR_DATE_LENGTH } from '../constants/date';
import { DayInfo, YearMonth, YearMonthDate } from '../types/date';

/** 2023-11-12 포맷인 날짜 문자열을 Date로 변경해 주는 함수 */
export const toDate = (dateString: string) => {
  return new Date(dateString);
};

/** Date를 2023-11-12 포맷인 날짜 문자열로 변경해 주는 함수 */
export const formatDate = ({ year, month, date }: YearMonthDate) => {
...
/** 현재 날짜가 오늘 이전 날짜인지에 대한 여부를 알려주는 함수 */
const isDayInPast = (dateString: string, endDateString: string | null) => {
  if (!endDateString) return false;
...
export const getDayInfo = ({
  ...
  const dateString = formatDate({ year: yearMonthData.year, month: yearMonthData.month, date });
  ...
  const isStart = dateRange?.startDate === dateString;
  const isEnded = dateRange?.endDate === dateString;
  const isInRange =
    !!dateRange?.startDate &&
    !!dateRange?.endDate &&
  ...
  const isRestricted =
    ...
    (isPastDaysRestricted && isDayInPast(dateString, todayDateString));
  return {
    date,
    isDate,
    dateString,
